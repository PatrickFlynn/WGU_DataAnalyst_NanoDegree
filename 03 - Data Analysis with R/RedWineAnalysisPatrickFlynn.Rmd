...
summary(wine)
test <- ggplot(data=wine, aes(x=residual.sugar)) + geom_histogram(color='black', fill='#891e1e') + labs(x="Residual Sugar", title="test!")
test
ggplot(data=wine, aes(x= fixed.acidity, y = volatile.acidity)) + geom_point() + geom_point(color='red', stat='summary', fun.x = mean)
```
...
```{r, echo=FALSE} 
winequal <- ggplot(data=wine, aes(x=quality, label=..count..)) + geom_histogram(color='black', fill='#891e1e', binwidth=1, na.rm=TRUE) + labs(title="Counts by Quality of Wine") +scale_x_continuous(name="Quality of Wine", limits=c(2, 9), breaks = seq(3,8, 1)) + theme_classic()
winequal
```
...
```{r, echo=FALSE} 
winequal = ggplot(data=wine, aes(x=alcohol)) + geom_histogram(color='black', fill='#891e1e', binwidth=0.25, na.rm=TRUE) + labs(title="Alcohol Content Measurements") +scale_x_continuous(name="Alcohol Content (%)", limits=c(8, 15.5), breaks = seq(0,18, 0.5)) 
winequal
```
...
ggplot(data=melted_matrix, aes(x=Var1, y=Var2, fill=value)) + geom_tile() + scale_fill_gradient(low="white", high="#891e1e") + theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
